...
- **Craftsman best practices:** Always consult the Context7 library **`/greensock/react`** via the MCP inspector (`npx -y @modelcontextprotocol/inspector --cli npx @upstash/context7-mcp`) before you touch visual design; that archive details how the `useGSAP` hook keeps animation setup and cleanup pristine.
